...
// 주문 데이터 제출
const submit = async () => {
  if (state.form.payment !== "card") { // 결재 수단이 카드가 아니면
  ...
  if (res.status === 200) {
    ...
    if (state.form.payment === "bank") {
      ...
      message.push(`한국은 123-456789-777 계좌로 ${price}원을 입금해주시기 바랍니다.`);
    }
      ...
      <div class="row g-5">
        ...
        <div class="col-md-7 col-lg-8">
          ...
          <div class="my-3">
            <div class="form-check">
              <input type="radio" class="form-check-input" id="card" name="paymentMethod" value="card" v-model="state.form.payment">
              <label for="card" class="form-check-label">신용카드</label>
            </div>
            <div class="form-check">
              <input type="radio" class="form-check-input" id="bank" name="paymentMethod" value="bank" v-model="state.form.payment">
        ...
        <div class="col-md-5 col-lg-4">
          ...
          <ul class="items list-group mb-3">
            <li class="p-3 list-group-item d-flex justify-content-between" v-for="i in state.items" :key="i.id">
              <div>
